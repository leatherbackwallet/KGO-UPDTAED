"use strict";
/**
 * Admin Categories Routes - MongoDB-only category management
 * Handles all CRUD operations for categories in admin dashboard
 * Always uses MongoDB - no JSON file dependency
 */
...
// GET /api/admin/categories - Get all categories from MongoDB
router.get('/', (0, requestBatching_1.deduplicateRequests)(), cache_1.cacheConfigs.categories, database_1.ensureDatabaseConnection, async (req, res) => {
    try {
        ...
        console.log('🔧 Admin categories request - using MongoDB with filter:', filter);
...
router.post('/', auth_1.auth, (0, role_1.requireRole)('admin'), database_1.ensureDatabaseConnection, (0, validation_1.validate)(createCategorySchema), async (req, res) => {
    try {
        ...
        (0, cache_1.invalidateCache)('/api/admin/categories');
...
// PUT /api/admin/categories/:id/products - Assign products to category
