...
exports.migrateOccasions = migrateOccasions;
...
async function migrateOccasions() {
    try {
        ...
        const products = await products_model_1.Product.find({ occasions: { $exists: true, $ne: [] } });
        console.log(`📦 Found ${products.length} products with occasions`);
        let migratedCount = 0;
        for (const product of products) {
            const newOccasionIds = [];
            for (const oldOccasion of product.occasions) {
                const newOccasionName = OCCASION_MAPPING[oldOccasion.toString()];
            ...
            if (newOccasionIds.length > 0) {
                product.occasions = newOccasionIds.map(id => new mongoose_1.default.Types.ObjectId(id));
                await product.save();
                migratedCount++;
