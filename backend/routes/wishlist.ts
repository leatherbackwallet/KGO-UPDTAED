...
router.get('/', auth, async (req: Request, res: Response): Promise<void> => {
  try {
    const wishlist = await Wishlist.findOne({ userId: (req as any).user.id })
      .populate({
        path: 'products',
...
router.post('/add/:productId', auth, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    let wishlist = await Wishlist.findOne({ userId: (req as any).user.id });
...
router.get('/check/:productId', auth, async (req: Request, res: Response): Promise<void> => {
  try {
    const { productId } = req.params;
...
router.post('/merge', auth, async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const guestProductIds = items.map((item: any) => item.product);
    const existingProductIds = wishlist.products.map((id: any) => id.toString());
    
    // Only add items that don't already exist
    const newProductIds = guestProductIds.filter((id: string) => !existingProductIds.includes(id));
    ...
    console.log(`Merged ${newProductIds.length} wishlist items for user ${(req as any).user.id}`);
