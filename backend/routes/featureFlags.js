 ...
 */
router.put('/:flagName', auth, role(['admin']), (req, res) => {
  try {
    const { flagName } = req.params;
 ...
 */
router.post('/:flagName/rollback', auth, role(['admin']), (req, res) => {
  try {
    const { flagName } = req.params;
 ...
 */
router.get('/:flagName/metrics', auth, role(['admin']), (req, res) => {
  try {
    const { flagName } = req.params;
...
router.get('/health/check', (req, res) => {
  try {
    ...
    const rollingOutFlags = Object.values(featureFlags.flags).filter(
      flag => flag.enabled && flag.rolloutPercentage > 0 && flag.rolloutPercentage < 100
    ).length;
